...
import { ConnectionStatus, CLIENT_IDENTITY } from "@/lib/mcp/constants";
import { Notification } from "@/lib/mcp/notificationTypes";
import {
  auth,
...
} from "@/lib/mcp/utils/configUtils";
import { getMCPServerRequestTimeout } from "@/lib/mcp/utils/configUtils";
import { InspectorConfig } from "@/lib/mcp/configurationTypes";
import { Transport } from "@modelcontextprotocol/sdk/shared/transport.js";
import { CustomHeaders } from "@/lib/mcp/types/customHeaders";
import { resolveRefsInMessage } from "@/lib/mcp/utils/schemaUtils";
...
export function useConnection({
  ...
  const [oauthTriggered, setOauthTriggered] = useState(false);
  ...
  const handleAuthError = async (error: unknown): Promise<{ handled: boolean; oauthTriggered: boolean }> => {
    if (is401Error(error)) {
      ...
      // If result is not "AUTHORIZED", OAuth popup was likely opened
      const oauthTriggered = result !== "AUTHORIZED";
      return { handled: true, oauthTriggered };
    }

    return { handled: false, oauthTriggered: false };
  };
    ...
    try {
      ...
      try {
        ...
        if (authResult.handled) {
          setOauthTriggered(authResult.oauthTriggered);
          if (authResult.oauthTriggered) {
            // OAuth popup was opened, don't set error state yet
